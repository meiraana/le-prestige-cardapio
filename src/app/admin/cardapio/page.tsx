...
export default function GestaoCardapioPage() {
  ...
  const pratosFiltrados =
    categoriaAtiva === 'TODOS'
      ? menuItems
      : menuItems.filter(
          (p) => p.category.toUpperCase() === categoriaAtiva.toUpperCase()
        );
  ...
  const handleSavePrato = (e: React.FormEvent) => {
    ...
    if (editingId !== null) {
      const pratoExistente = menuItems.find((p) => p.id === editingId);
      atualizarPrato({
        id: editingId,
        name: nome,
        ...
        image: imagem || pratoExistente?.image || '/img/placeholder.png',
      });
    } else {
      adicionarPrato({
        name: nome,
        ...
        category: categoria,
        available: disponivel,
        image: imagem.trim() || '/img/placeholder.png',
      });
    }
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    value={categoria}
                    onChange={(e) =>
                      setCategoria(e.target.value as MenuItem['category'])
                    }
                    className="w-full bg-[#EAE8E1]/60 border border-verde rounded-lg px-3 py-2 text-sm text-cafe focus:outline-none focus:border-verde"
                  >
                    {CATEGORIES.map((cat) => (
